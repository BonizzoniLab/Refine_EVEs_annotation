...
def read_blast_table(blastx_file):
    ...
    for line in blastx_file:
        ...
        accession_s_id=parts[3]
        instance.s_id=' '.join(accession_s_id.split(' ')[1:])
        #instance.s_id=parts[3]
        instance.accession=accession_s_id.split(' ')[0]
        instance.evalue=float(parts[4])
        instance.q_frame=int(parts[5])
        instance.perc_id=float(parts[6])
        instance.q_perc_cov=float(parts[7])
        instance.s_start=parts[8]
        instance.s_end=parts[9]
        instance.s_len=parts[10]
        try:
            instance.taxid=parts[11]
        except:
            instance.taxid=''
